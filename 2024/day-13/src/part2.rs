...
pub fn process(input: &str) -> Result<String> {
    ...
    let result = machines.iter().map(calculate_tokens).sum::<u64>();

    Ok(result.to_string())
}

fn calculate_tokens(machine: &Machine) -> u64 {
    ...
    a.trunc() as u64 * 3 + b.trunc() as u64
}
...
fn process_prize(input: &str) -> IResult<&str, Prize> {
    const ADJUSTMENT: u64 = 10000000000000;
