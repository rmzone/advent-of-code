...
fn is_valid(
    ...
    for y in start_y..end_y + 1 as usize {
        for x in start_x..end_x + 1 as usize {
            let position = x + y * width as usize;
            if let Some(c) = matrix.get(position) {
                if *c == '*' {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let mut part_numbers: Vec<Gear> = vec![];
    ...
    for y in 0..height as usize {
        for x in 0..width as usize {
            if last_row != y {
                ...
                if let Some(gear) =
                    check(&matrix, &mut part_number, start_x, start_y, width, height)
                {
                    part_numbers.push(gear);
                }
                part_number.clear();
                last_row = y;
            }
            ...
            let len = &part_number.len();
            if let Some(gear) = check(&matrix, &mut part_number, x - len, y, width, height) {
                part_numbers.push(gear);
            }
            part_number.clear();
        }
    ...
    // group all the valid gears together
    let mut gears: BTreeMap<usize, Gear> = BTreeMap::new();

    for gear in part_numbers.iter() {
        for gear2 in part_numbers.iter() {
            if gear != gear2 && gear.id == gear2.id {
                // add to map
                if !gears.contains_key(&gear.id) {
                    gears.insert(
                        gear.id,
                        Gear {
                            id: gear.id,
                            part_number: format!(
                                "{}",
                                gear.part_number.parse::<u32>().unwrap()
                                    * gear2.part_number.parse::<u32>().unwrap()
                            )
                            .to_string(),
                        },
                    );
    ...
    let sum = gears
        .iter()
        .map(|gear| {
            let i = gear.1.part_number.parse::<u32>().unwrap();
            i
        })
        .sum::<u32>();
    Ok(sum.to_string())
