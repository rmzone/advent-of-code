    ...
    let (input, grid) = many1(delimited(
        ...
        alt((
            tag("/").map(|span| {
                ...
                Mirror {
                    mirror_type: MirrorType::ForwardSlash,
                    span: located,
                }
            }),
            tag(r#"\"#).map(|span| {
                let located = with_xy(span);
                Mirror {
                    mirror_type: MirrorType::BackSlash,
                    span: located,
                }
            }),
            tag("-").map(|span| {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let grid_map = grid
        .iter()
        .map(|mirror| (mirror.span.extra, mirror.mirror_type))
...
fn run(starting_direction: Direction,
       starting_position: IVec2,
       row_count: i32,
       column_count: i32,
       grid_map: &HashMap<IVec2, MirrorType>) -> usize {
        ...
        for (direction, current_position) in active_beams.iter() {
            ...
            loop {
                ...
                match march(direction, grid_map.get(&new_position)) {
                    ...
                    March::Split(d1, d2) => {if !all_movement.iter().any(
                        |item| {
                    ...
                    March::Dir(d) => {if !all_movement.iter().any(
                        |item| {
                            item == &(d, new_position)
                        }
                    ) {
