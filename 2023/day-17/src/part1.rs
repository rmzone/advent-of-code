...
fn a_number(input: Span) -> IResult<Span, (IVec2, u32)> {
    ...
    Ok((input, (IVec2::new(x, y), n.to_digit(10).unwrap())))
}

fn parse_grid(input: Span) -> IResult<Span, HashMap<IVec2, u32>> {
    let (input, output) = separated_list1(line_ending, many1(a_number))(input)?;
    let grid = output
        .into_iter()
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let column_count = input.lines().next().unwrap().len() as i32; // assumes all the lines are the same length
