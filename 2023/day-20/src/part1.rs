...
fn flip_flop(input: &str) -> IResult<&str, Machine> {
    ...
    Ok((input, Machine {
        machine_type: MachineType::FlipFlop { status: Status::Off },
        id: id,
        output: outputs,
    }))
}
...
fn conjunction(input: &str) -> IResult<&str, Machine> {
    ...
    Ok((input, Machine {
        machine_type: MachineType::Conjunction { memory: HashMap::new() },
        id: id,
        output: outputs,
    }))
}

fn parse_input(input: &str) -> IResult<&str, HashMap<&str, Machine>> {
    let (input, machines) = separated_list1(
        line_ending,
        alt((broadcast, flip_flop, conjunction)))(input).unwrap();
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let (input, mut machines) = parse_input(input).unwrap();
