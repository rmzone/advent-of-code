...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    // list of the matches for each card
    let data = cards
        .iter()
        .map(|card| card.num_matches())
        .collect::<Vec<_>>();

    // build a base map with the number of card instances for each card. i.e. one each to start
...
Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11";
        assert_eq!("30", crate::part2::process(input)?);
        Ok(())
    }
}
