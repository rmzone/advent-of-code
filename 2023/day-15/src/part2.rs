...
impl<'a> TryFrom<&Instruction<'a>> for Lens<'a> {
    ...
    fn try_from(value: &Instruction<'a>) -> Result<Self, Self::Error> {
        match value.operation {
            ...
            Operation::Remove => Err(format!("invalid conversion for {}", value.label)),
        }
    }
}
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    let (_, ins) = instructions(input).expect("should parse");

    let boxes: Vec<Vec<Lens>> = (0..256).into_iter().map(|_| vec![]).collect();

    let filled_boxes = ins.iter().fold(boxes, |mut boxes, next_instruction| {
        match Lens::try_from(next_instruction) {
            Ok(lens) => {
                // insert or replace (find label in specific box)
                let index = boxes[next_instruction.hash as usize]
                    .iter()
                    .position(|a_lens| a_lens.label == lens.label);
                match index {
                    Some(lens_index) => {
                        // replace
                        let _ = std::mem::replace(
                            &mut boxes[next_instruction.hash as usize][lens_index],
                            lens,
                        );
                    }
                    None => {
    ...
    let result = filled_boxes
        ...
        .flat_map(|(box_position, r#box)| {
            r#box.into_iter().enumerate().map(move |(position, lens)| {
                let result = (box_position + 1) * (position + 1) * (lens.focal_length as usize);
